...
## Update: clearer enemy telegraphs + support fields + mobile controls
- Split hostile telegraph rendering from active enemy attack rendering so warning overlays stay on top of player skill VFX.
- Reworked hostile telegraphs with stronger contrast, animated dash flow, pop-in, and visible fill buildup for circular zones and beams.
- Replaced enemy support/buff burst ring with a distinct support-field look so banner/mortar empowerment reads differently from incoming damage.
- Added mobile touch controls:
  - compact mobile HUD layout that leaves the center playfield clearer,
  - virtual joystick with a larger hidden capture zone so drag can start near the stick rather than exactly on the drawn base,
  - on-screen dash button.
- Exposed touch-control state in `render_game_to_text` for future verification scripts.

## Latest verification
- `npm run check` passes after the telegraph/mobile pass.
- `npm run playtest:smoke` passes after the telegraph/mobile pass.
- Attempted an additional mobile-specific Playwright verification, but the local project does not ship a directly importable `playwright` module for the inline check flow used here, so mobile touch interaction was verified by code path inspection and smoke coverage rather than a dedicated mobile drag script.
...
## Update: hosted/browser resolution stability pass (2026-04-10)
- Added adaptive internal canvas resolution for the main game render:
  - logical viewport size still tracks the real browser window,
  - internal render buffer now scales down on larger screens or weaker hardware instead of always drawing every pixel at full window resolution.
- Added a lightweight hardware/perf-based render budget using `navigator.deviceMemory`, `navigator.hardwareConcurrency`, viewport area, and the existing performance tier.
- Enabled `desynchronized: true` on the main 2D canvas context when available.
- Exposed the active render scale and internal canvas size through `render_game_to_text` for verification and future profiling.

## Verification run (hosted/browser stability pass)
- `npm run check` passes.
- `npm run playtest:smoke` passes.
- Smoke artifact check:
  - `output/web-game/playtest-smoke/shot-0.png`
  - `output/web-game/playtest-smoke/state-0.json`
- `state-0.json` now includes `render.scale`, `render.width`, `render.height`, `render.viewportWidth`, `render.viewportHeight`.
